...
> [!IMPORTANT]
> 
> VercelのOSSスポンサーシップに対する最近の変更により、公式の `github-readme-stats.vercel.app` デプロイは、近い将来、一時停止または終了する可能性があります。
> 
> サービスの中断を防ぐため、ご自身のVercelアカウントにインスタンスをデプロイすることを強くお勧めします。詳細な手順については、「[自分の Vercel インスタンスにデプロイする](#自分の-vercel-インスタンスにデプロイする)」セクションをご覧ください。
